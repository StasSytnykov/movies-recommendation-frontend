...
export const useLangugaeChange = ({
  sortedByQuery,
  setSortedByQuery,
}: UseLanguageChange) => {
  ...
  const sortedByPopularity =
    locale === "uk" ? SORTED_BY_POPULARITY_UA : SORTED_BY_POPULARITY;
  const sortedByReleaseDate =
    locale === "uk" ? SORTED_BY_RELEASE_DATE_UA : SORTED_BY_RELEASE_DATE;
  const sortedByRating =
    locale === "uk" ? SORTED_BY_RATING_UA : SORTED_BY_RATING;

  React.useEffect(() => {
    if (SORTED_BY_POPULARITY_UA === sortedByQuery && locale === "en-us") {
      setSortedByQuery(SORTED_BY_POPULARITY);
    }
    if (SORTED_BY_POPULARITY === sortedByQuery && locale === "uk") {
    ...
    if (SORTED_BY_RELEASE_DATE === sortedByQuery && locale === "uk") {
    ...
    if (SORTED_BY_RATING === sortedByQuery && locale === "uk") {
